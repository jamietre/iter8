...
```
Invokes the `Constructor` passed with the sequence. This works well with the ES6 `Map` and `Set` types. You can also use `Array`, which is a special case, even though constructing an Array with an iterable doesn't work directly. You can use any user-defined types that can accept an iterable as a single constructor argument.

#### toArray()

Return the sequence as an `Array`. Same as `as(Array)`

#### toObject()

Assuming the seqeunce contains *key-value pairs* and each key is unique, return an object with a property: value for each key-value pair.

This method makes no effort to validate your sequence, rather, it just uses `value[0]` for each key and `value[1]` for each value. It is useful especially in conjunction with `groupBy`, which returns a sequnce of key/value pairs, or when `iter` is created from a `Map` object.
...
```Javascript
    let x = iter([1,2,3,4,5]).count() // x === 5
```
...
Every `Array` method that doesn't mutate the array is supported, and execution is deferred until the query is executed. The signature is the same as the native, except arguments which pass the array itself to a callback are not implemented.

`length` is also explcitly not implemented. Instead see `count`, above. Since counting the elements in a seqeunce requires iterating the sequence, this is a method rather than a property.
...
There are opportunities for code reuse that I haven't taken advantage of that could reduce the size of this library.
