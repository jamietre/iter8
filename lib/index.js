...
Iter[_p] = {
    ...
    flatten: makeIterable(function(get, done, recurse) {
        var iterators = [];
        var iterator = get();
        var open = this[_open]
        return function() {
            while (iterator || iterators.length > 0) {
                ...
                if (cur.value && cur.value[_iterator] && typeOf(cur.value) !== _s && (recurse || iterators.length === 0)) {
     ...
     */    
    map: checkArgs(newIter(makeMapIterator),[_gk]),
    /**
     ...
     * @returns {number} The index or -1
     */
    findIndex: checkArgs(makeAggregator('var i=0','if (a.call(b,{v},i)===true) { z([_i]); return i } i++','return -1'), [_f]),
    /**
    ...
    return function() {
        return new Iter(_iterator, 
            generator.apply(
                this, 
                (args || []).concat(
                    slice(arguments,0,nargs || undefined)
                )
            )
        );
    }
...
function newCachedIter(generator, nargs) {
    return function() {
        return new Iter(_iterator, generator, this, slice(arguments, 0, nargs));
    }
}
        ...
        return that[_withReturn](function() {
            ...
            while (true) {
                if (!matches) {
                    ...
                    if (!match || !match[_iterator] || typeOf(match) === _s) {
                        that[_open] = [iterator]
                        return { 
                            done: false, 
...
function makeIterable(fn) {
    return newIter(function() {
        var that = this;
        var args = arguments;
        return function() {
...
function makeFlattenIterator(recurse) {
    ...
    return function() {
        var iterators = [];
        var iterator = that[_get]();
         
        return that[_withReturn](function() {
            while (iterator || iterators.length > 0) {
                if (!iterator) {
                    iterator = iterators.pop();
                    if (that[_open].indexOf(iterator)>0) that[_open].push(iterator)
                }
...
function verifyCb(cb) {
    if (typeOf(cb) !== _f) throw new Error('The callback argument was not a function.')
}
