 ...
 */
declare function iter<T>(source: Iterable<T> | Iterator<T> | Generator<T>): Iter<T>;
...
declare type Provider<T> = (item: T) => T;
declare type ProviderWithIndex<T> = (item: T, index: number) => T;
declare type KeyProvider<T> = Provider<T> | string;
declare type KeyProvider2<T> = ProviderWithIndex<T> | string;

declare type TestProvider<T> = (item: T) => boolean;
declare type TestProviderWithIndex<T> =  (item: T, index: number) => boolean
...
export declare class IterKvp<K,V> extends Iter<[K, V]> {
    /**
     * Assuming the seqeunce contains [key, value] pairs, return a sequence of the keys
     * 
     * @returns {Iter<K>} the keys
